...
app.get("/3cx/config", async (req, res) => {
  try {
      const response = await axios.get("https://1719.3cx.cloud/MyPhone/config/rewitch");
      res.json(response.data);
  } catch (error) {
      res.status(500).json({ error: "Error fetching data from 3CX" });
  }
});
